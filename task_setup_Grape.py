...
def setup_task_environment(self):
    ...
    '''
    home=Pose(Point(0.0,0.0,0.0),Quaternion(0,0,0,1))
    # Create a list to hold the waypoint poses
    self.waypoints = list()
            
    # Append each of the four waypoints to the list.  Each waypoint
    # is a pose consisting of a position and orientation in the map frame.
    self.waypoints.append(Pose(Point(3.06, 1, 0.0), Quaternion(0,0,-0.67,0.735)))
    self.waypoints.append(Pose(Point(0.76, 0.82, 0.0), Quaternion(0,0,-0.99,0.10)))
    self.waypoints.append(Pose(Point(2.89, 1.8, 0.0), Quaternion(0,0,0.68,0.735)))
    self.waypoints.append(Pose(Point(-.70, 1.40, 0.0), Quaternion(0,0,-0.67,0.735)))
    
    # Create a mapping of room names to waypoint locations
    farm_locations = (('hallway', self.waypoints[0]),
    ...
    self.farm_locations = OrderedDict(farm_locations)
...
def init_waypoint_markers(self):
    ...
    marker_ns = 'waypoints'
    marker_id = 0
    marker_color = {'r': 1.0, 'g': 0.0, 'b': 1.0, 'a': 1.0}
